#!/bin/bash
# ------------------------------------------------------------------
# Forensic Tool One Installation Script
# ------------------------------------------------------------------
# This script installs Forensic Tool One by copying the necessary
...
INSTALL_DIR="/usr/local/forensic_tool_one"

echo "Starting installation of Forensic Tool One..."

# Check if running as root (if necessary for the installation directory)
if [ "$EUID" -ne 0 ]; then
  echo "Please run as root or use sudo to install Forensic Tool One."
  exit 1
fi
...
echo "Forensic Tool One installed successfully."
exit 0
